...
// Initialize notification service (graceful if Firebase credentials not set)
const _fbCreds = {
    projectId:   process.env.FIREBASE_PROJECT_ID,
    clientEmail: process.env.FIREBASE_CLIENT_EMAIL,
    privateKey:  process.env.FIREBASE_PRIVATE_KEY?.replace(/\\n/g, '\n'),
};
console.log(`[Firebase] Credentials check — projectId=${_fbCreds.projectId || 'MISSING'} clientEmail=${_fbCreds.clientEmail ? 'SET' : 'MISSING'} privateKey=${_fbCreds.privateKey ? `SET(${_fbCreds.privateKey.length}c)` : 'MISSING'}`);
notificationService.init(_fbCreds);
const _ns = notificationService.getStatus();
console.log(`[Firebase] Notification service: ready=${_ns.ready}${_ns.initError ? ' error=' + _ns.initError : ''}`);
// Expose for use in /api/notifications/test route
...
// Serve PWA assets (manifest, service worker, icons) — must come before dist/
// In pkg EXE mode, public/ is not reliably in the snapshot, so we use the
// pre-bundled publicAssets module (generated by generate-public-assets.cjs).
...
// Optional secret auth on all /api/notifications/* routes
...
try { tokenStore.pruneInactive(30); } catch (_) {}
...
app.post('/api/notifications/register', async (req, res) => {
    ...
    try {
        const { token, deviceName, userAgent } = req.body || {};
        if (!token || typeof token !== 'string') {
            return res.status(400).json({ error: 'token is required' });
        }
        const entry = tokenStore.upsertToken({ token, deviceName, userAgent });
        res.json({ id: entry.id, registered: true });
    } catch (err) {
...
// GET /api/notifications/devices — list registered devices (token string excluded)
app.get('/api/notifications/devices', (req, res) => {
    try {
        const devices = tokenStore.getTokens().map(({ id, deviceName, userAgent, registeredAt, lastSeenAt, active }) => ({
            id, deviceName, userAgent, registeredAt, lastSeenAt, active
        }));
        res.json({ devices });
    } catch (err) {
...
// POST /api/notifications/test — send a test notification to a token
app.post('/api/notifications/test', async (req, res) => {
    const tokenKey = (req.body?.token || '').slice(0, 20);
    if (!checkRateLimit(`test:${tokenKey}`, 3, 60_000)) {
        return res.status(429).json({ error: 'Too many requests' });
    }
    try {
        const { token } = req.body || {};
        if (!token) return res.status(400).json({ error: 'token is required' });
        // notificationService wired in Phase A2 — graceful stub until then
        if (global.notificationService) {
            await global.notificationService.sendTest(token);
        }
        res.json({ sent: true });
    } catch (err) {
...
app.get('/api/notifications/history', (req, res) => {
    try {
        ...
        if (!fs.existsSync(historyFile)) return res.json({ total: 0, entries: [] });
        const { entries = [] } = JSON.parse(fs.readFileSync(historyFile, 'utf8'));
        const slice = entries.slice(offset, offset + limit);
        res.json({ total: entries.length, entries: slice });
    } catch (err) {
...
app.get('/api/notifications/settings', (req, res) => {
    try {
        const settings = stateService.get(NOTIFICATION_SETTINGS_KEY);
        res.json(settings || DEFAULT_NOTIFICATION_SETTINGS);
    } catch (err) {
...
// GET /setup — serve the mobile device registration page (never cached)
app.get('/setup', (req, res) => {
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.setHeader('Pragma', 'no-cache');
    res.setHeader('Expires', '0');
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    const content = publicAssets['setup.html']?.content;
    if (content) return res.send(content);
    // Dev fallback: read from real filesystem
    try {
        res.send(fs.readFileSync(path.join(__dirname, 'public', 'setup.html'), 'utf8'));
    } catch (err) {
...
app.get('/api/notifications/setup-url', async (req, res) => {
    try {
        const qrcode = require('qrcode');
        const HTTPS_PORT_VAL = parseInt(process.env.HTTPS_PORT || '3443', 10);
...
app.get('/api/notifications/status', (req, res) => {
    ...
    console.log('[NotificationService] Status check:', status);
    res.json(status);
});

// POST /api/notifications/test-device — send test notification to a specific device by id
app.post('/api/notifications/test-device', async (req, res) => {
    try {
        const { deviceId } = req.body || {};
        if (!deviceId) return res.status(400).json({ error: 'deviceId required' });
        const tokens = tokenStore.getTokens();
        const device = tokens.find(t => t.id === deviceId);
        if (!device) return res.status(404).json({ error: 'Device not found' });
        if (!notificationService) return res.status(503).json({ error: 'Notification service not available' });
        await notificationService.sendTest(device.token);
        res.json({ sent: true });
    } catch (err) {
...
app.get('*', (req, res) => {
    ...
    // Never cache index.html — stale cache + service worker can cause broken soft-reloads
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.setHeader('Pragma', 'no-cache');
    res.setHeader('Expires', '0');
...
server.listen(PORT, () => {
    ...
    // Start HTTPS server + tunnel in background (async)
    const HTTPS_PORT = parseInt(process.env.HTTPS_PORT || '3443', 10);
    ;(async () => {
        // 1. Self-signed HTTPS for LAN (cert covers current LAN IPs)
        try {
            const lanIPs = ipDetector.getLANIPs();
            console.log(`[HTTPS] Generating cert for IPs: ${lanIPs.join(', ')} …`);
            ...
            if (sslCert) {
                ...
                console.warn('[HTTPS] Cert generation failed — HTTPS server not started');
            }
        } catch (err) {
    ...
    console.log(`  Device Setup:   https://${primaryIP}:${HTTPS_PORT_VAL}/setup`);
    ...
    const _nsStatus = notificationService.getStatus();
    console.log(`  Notifications: ${_nsStatus.ready ? '✅ Ready' : '❌ NOT ready — ' + (_nsStatus.initError || 'check .env Firebase credentials')}`);
    console.log('');
